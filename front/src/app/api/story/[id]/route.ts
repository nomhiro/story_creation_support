...
export const GET = async (_: NextRequest,{ params }:{params: {id: string}}) => {
  try {
    ...
    const { resources } = await getContainer('story').items.query(querySpec).fetchAll();
...
export const DELETE = async (_: NextRequest,{ params }:{params: {id: string}}) => {
  try {
    ...
    const { resources } = await getContainer('story').items.query({
      query: `SELECT * FROM c WHERE c.id = "${params.id}"`,
    }).fetchAll();
    ...
    await getContainer('story').item(item.id, item.partitionKey).delete();
